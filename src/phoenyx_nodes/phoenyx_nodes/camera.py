...
def obtener_num2(imagen, log_level = 0):# no
    try:
...
def cuadrado(frame, color, text, confianza = 0):# no
    square_size = 100 
...
def image_to_number(image):
    try:
        ...
        config = '--psm 10 -c tessedit_char_whitelist=0123456789'  # se ha quitado el --oem 3
        number =  pytesseract.image_to_string(new_image, config = config) 
...
def leer_imagenes_de_la_camara(index):
    ...
    # cap.set(cv2.CAP_PROP_BRIGHTNESS, 30)  # Brillo (valor entre 0 - 255)
    # cap.set(cv2.CAP_PROP_CONTRAST, 150)     # Contraste (valor entre 0 - 255)
    # cap.set(cv2.CAP_PROP_EXPOSURE, -6)     # Exposición (valores negativos = auto)
    # cap.set(cv2.CAP_PROP_GAIN, 20)         # Ganancia (valor recomendado 0-255)
    cap.set(cv2.CAP_PROP_AUTO_EXPOSURE, 1) # 0=Manual, 1=Automático
